...
protected:
    ...
    virtual int set_nonbloking_mode() const noexcept {
        ...
        return fcntl( obj->fd, F_SETFL, flags | _O_NONBLOCK );
    }
    
    /*─······································································─*/

    uint get_fd_flag( const string_t& flag ){ uint _flag = _O_NONBLOCK;
             if( flag == "r"  ){ _flag |= _O_RDONLY | _O_CREAT; }
        else if( flag == "w"  ){ _flag |= _O_WRONLY | _O_CREAT; }
        else if( flag == "a"  ){ _flag |= _O_APPEND | _O_CREAT; }
        else if( flag == "r+" ){ _flag |= _O_RDWR   ;          }
        else if( flag == "w+" ){ _flag |= _O_RDWR   | _O_CREAT; }
        else if( flag == "a+" ){ _flag |= _O_APPEND ;          }
        else                   { _flag |= _O_RDONLY | _O_CREAT; }
        return  _flag;
    }
...
public: file_t() noexcept {}
    ...
    file_t( const int& fd, const ulong& _size=CHUNK_SIZE ){
        if( fd < 0 ) $Error("such file or directory does not exist");
            obj->fd = fd; 
        set_nonbloking_mode(); set_buffer_size( _size );
    }
