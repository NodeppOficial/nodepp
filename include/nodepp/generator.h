...
namespace nodepp { namespace _encode_ {
    ...
    _Generator(pipe) { public:
        ...
        _Emit( const string_t& from, const string_t& to, const T& inp, const V& out, ulong mult=1 ){
        _GStart  ctx = iconv_open( to.data(), from.data() ); 

            if( ctx == (iconv_t)-1 ){
                _EError(inp.onError,"can't open a encode context");
                _EError(out.onError,"can't open a encode context"); _End;
            }
        ...
        _GStart ctx = iconv_open( to.data(), from.data() ); 
            ...
            if( ctx == (iconv_t)-1 ){
                _EError(inp.onError,"can't open a encode context"); _End;
            }
