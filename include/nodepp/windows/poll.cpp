...
public: poll_t() noexcept : obj( new _str_() ) {}
    ...
    _GStart
        ...
        if( obj->ev.empty() )                                  { _End; }
        if( ::WSAPoll( obj->ev.data(), obj->ev.size(), 0 )<=0 ){ _End; } s = obj->ev.size(); 

        while( s-->0 ){ x = obj->ev[s]; 
              if( x.revents & POLLERR ){ obj->ev.erase(s); onError.emit(x.fd); obj->ls={{-1, (int)x.fd }}; _Next; }
            elif( x.revents & POLLIN  ){ obj->ev.erase(s);  onRead.emit(x.fd); obj->ls={{ 0, (int)x.fd }}; _Next; }
            elif( x.revents & POLLOUT ){ obj->ev.erase(s); onWrite.emit(x.fd); obj->ls={{ 1, (int)x.fd }}; _Next; }
        }
