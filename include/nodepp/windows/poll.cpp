...
namespace nodepp { using POLLFD = WSAPOLLFD; class poll_t {
protected:
   ...
   ~poll_t() noexcept { 
    ...
    coStart
        ...
        while( s-->0 ){ x = obj->ev[s]; 
              if( x.revents & POLLERR ){ obj->ev.erase(s); onError.emit(x.fd); obj->ls={{-1, (int)x.fd }}; onEvent.emit(obj->ls); coNext; }
            elif( x.revents & POLLIN  ){ obj->ev.erase(s);  onRead.emit(x.fd); obj->ls={{ 0, (int)x.fd }}; onEvent.emit(obj->ls); coNext; }
            elif( x.revents & POLLOUT ){ obj->ev.erase(s); onWrite.emit(x.fd); obj->ls={{ 1, (int)x.fd }}; onEvent.emit(obj->ls); coNext; }
        }
